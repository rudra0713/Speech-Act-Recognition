...
class Decoder(nn.Module):
    def __init__(self, output_dim, emb_dim, enc_hid_dim, dec_hid_dim, dropout, attention):
        ...
        self.rnn = nn.GRU(enc_hid_dim + output_dim, dec_hid_dim)
        self.fc = nn.Linear(enc_hid_dim + output_dim + dec_hid_dim, output_dim)
...
#         print("embed dim in decoder : ", embed.shape)
        ...
        #hidden = [batch size, dec hid dim]
        output, hidden = self.rnn(rnn_input.contiguous(), hidden.contiguous().unsqueeze(0))
...
class Seq2Seq(nn.Module):
    def __init__(self, decoder, device, d_model, heads, layers, num_sentences):
        super().__init__()
...
#         print(outputs.shape)
        ...
        encoder_outputs = self.transformer(src)
